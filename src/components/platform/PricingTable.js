    ...
    return (
        <div className="container px-3 py-2.5 mt-6">
            ...
            {#if pricing != "모든 장르"}
                <div className="mt-3 p-3 font-light rounded-md text-gray-200 bg-white bg-opacity-5">
                    <div className="text-xl">{pricing}</div>
                    <div className="mt-auto ml-auto">{pricingData[pricing]["novelCount"].toLocaleString()} 작품</div>
                    <div className="mt-2 grid grid-cols-2">
                        <div><span className="text-gray-400">조회수: </span>{pricingData[pricing]["totalViews"].toLocaleString()}</div>
                        <div className="ml-4"><span className="text-gray-400">평균 조회수: </span>{pricingData[pricing]["avgViews"].toLocaleString()}</div>
                    </div>
                    <div className="grid grid-cols-2">
                        <div><span className="text-gray-400">좋아요 수: </span>{pricingData[pricing]["totalLikes"].toLocaleString()}</div>
                        <div className="ml-4"><span className="text-gray-400">평균 좋아요 수: </span>{pricingData[pricing]["avgLikes"].toLocaleString()}</div>
                    </div>
                    <div className="grid grid-cols-2">
                        <div><span className="text-gray-400">선작수: </span>{pricingData[pricing]["totalFavs"].toLocaleString()}</div>
                        <div className="ml-4"><span className="text-gray-400">평균 선작수: </span>{pricingData[pricing]["avgFavs"].toLocaleString()}</div>
                    </div>
                </div>
            {/if}
        {/each} */}
